...
window.updateIdentifiers = function () {
				...
				type.endsWith(":base64binary")
					...
					: //convert base64 to base16 (hexBinary)
					  atob(
							...
							.map(function (aChar) {
								return (
									"0" + aChar.charCodeAt(0).toString(16)
								).slice(-2);
							})
							.join("")
							.toUpperCase()
...
window.setDynamicValues = function () {
	for (var i = 0; i < globalValuesMap.length; i++) {
		for (var j = 0; j < globalValuesMap[i].values.length; j++) {
			globalValuesMap[i].object
				.querySelector(
					"select option[value = '"
						.concat(globalValuesMap[i].values[j])
						.concat("']")
				)
				.setAttribute("selected", "selected");
		}
...
window.addEventListener("ubtuejk", function () {
	/* INITIAL CALLS */
	addHiddenFields();
	xmlToHTML(document);
	updateIdentifiers();
